...
const create_quiz_objekt = async (quiz, with_answer, view) => {
  try {
    ...
    quiz_object.required_points =
      quiz.getDataValue("required_points") * 100 + " %";
    ...
    for (const quiz_question of quiz_questions) {
      ...
      if (!view) {
        ...
        question_object.question_value =
          quiz_question.getDataValue("question_value");
      ...
      if (with_answer) {
        switch (quiz_question.category.toUpperCase()) {
          ...
          case QUESTIONTYPE.MULTIPLECHOICE:
            ...
            question_object.right_answer = q3
      ...
      if (
        quiz_question.category === QUESTIONTYPE.TRUEORFALSE ||
        quiz_question.category === QUESTIONTYPE.FILLINTHEBLANK
      ) {
      ...
      for (const option of options) {
        question_object.answers.push({
          id: idx,
          value: option.getDataValue("value"),
        });
        idx += 1;
      }
